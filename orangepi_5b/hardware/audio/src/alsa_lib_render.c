...
card 1: rockchipes8388 [rockchip-es8388], device 0: dailink-multicodecs ES8323.6-0010-0 [dailink-multicodecs ES8323.6-0010-0]
  Subdevices: 1/1
  Subdevice #0: subdevice #0
*/
...
int32_t AudioOutputRenderOpen(const struct DevHandle *handle, int cmdId, const struct AudioHwRenderParam *handleData)
{
    ...
    AUDIO_FUNC_LOGI("[Render] devName : %{public}s", cardIns->devName);
    ...
    AUDIO_FUNC_LOGI("[Render] ctrlName : %{public}s", cardIns->ctrlName);
...
int32_t AudioBindServiceObject(struct DevHandle *handle, const char *name)
{
    int32_t ret;
...
struct DevHandle *AudioBindService(const char *name)
{
    ...
    AUDIO_FUNC_LOGI("[Render] %{public}s", name);
    ...
    AUDIO_FUNC_LOGI("[Render] BIND SERVICE SUCCESS!");
