    ...
    const struct DevHandle *handle, int cmdId, struct AudioHwCaptureParam *handleData)
{
    ...
    AUDIO_FUNC_LOGI("Get Gain Threshold!");
...
int32_t AudioOutputCaptureOpen(const struct DevHandle *handle, int cmdId, const struct AudioHwCaptureParam *handleData)
{
    ...
    AUDIO_FUNC_LOGI("[Capture] devName : %{public}s", cardIns->devName);

    ret = snd_pcm_open(&cardIns->capturePcmHandle, "plughw:0,0", SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK);
    if (ret < 0) {
    ...
    AUDIO_FUNC_LOGI("[Capture] ctrlName : %{public}s", cardIns->ctrlName);
...
int32_t AudioBindServiceObject(struct DevHandle * const handle, const char *name)
{
    int32_t ret;
    char *serviceName;
...
struct DevHandle *AudioBindService(const char *name)
{
    ...
    AUDIO_FUNC_LOGI("[Capture] %{public}s", name);
    ...
    ret = AudioBindServiceObject(handle, name);
    if (ret != HDF_SUCCESS) {
        AUDIO_FUNC_LOGE("AudioBindServiceObject failed!");
    ...
    AUDIO_FUNC_LOGI("[Capture] BIND SERVICE SUCCESS!");
