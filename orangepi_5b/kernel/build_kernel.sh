...
KERNEL_SOURCE=${ROOT_DIR}/kernel/linux/linux-5.10.rk3588s
...
#hdf
echo "Patch hdf"
bash ${ROOT_DIR}/device/board/orangepi/orangepi_5b/kernel/patch_hdf.sh ${ROOT_DIR} ${KERNEL_SRC_TMP_PATH}
...
if [ "enable_ramdisk" == "${9}" ]; then
    ...
    ./make-ohos.sh ORANGEPI-5B disable_ramdisk
fi
