...
bool switch_ohos_boot_mode(void)
{
	...
	struct UpdateMessage *msg = NULL;
	OHOS_BOOT_MODE boot_mode = BOOT_NORMAL;
	dev_desc = rockchip_get_bootdev();
	if (dev_desc == NULL) {
		printf("Cannot get dev_desc\n");
		return false;
	}
	if (part_get_info_by_name(dev_desc, OHOS_PART_MISC, &part) < 0) { // 获取分区信息
        printf("Cannot find ohos misc partition\n");
        return false;
	}
	u32 cnt = DIV_ROUND_UP(sizeof(struct UpdateMessage), dev_desc->blksz);
	msg = memalign(ARCH_DMA_MINALIGN, cnt * dev_desc->blksz);
	if (blk_dread(dev_desc, part.start + MISC_UPDATE_MESSAGE_OFFSET, cnt, msg) != cnt) {
		printf("Failed to read from misc partition with %u block(s)\n", cnt);
		return false;
	}
	if (strcmp(msg->command, "boot_updater") == 0) {
		printf("Uboot enter Openharmony updater mode\n");
		boot_mode = BOOT_UPDATER;
	} else if (strcmp(msg->command, "boot_flash") == 0) {
		printf("Uboot enter Openharmony flashd mode\n");
		boot_mode = BOOT_FLASHD;
	}
	return boot_mode != BOOT_NORMAL ? true : false;
}
int load_openharmony_rd_to_ram(const char *rd_part)
{
	struct blk_desc *dev_desc = NULL;
	disk_partition_t part;
	if (rd_part == NULL || *rd_part == '\0') {
		printf("Error, load openharmony ramdisk with invalid partition name\n");
		return -1;
	}
	char *ram_addr_str = env_get("ramdisk_addr_r");
	if (ram_addr_str == NULL) {
		printf("Warning, cannot get env \'ramdisk_addr_r\'. set default to 0x0a200000\n");
		ram_addr_str = "0x0a200000";
	}
	dev_desc = rockchip_get_bootdev();
	if (dev_desc == NULL) {
		printf("Cannot get dev_desc\n");
	...
	if (part_get_info_by_name(dev_desc, rd_part, &part) < 0) { // 获取分区信息
        printf("Cannot find partition \' %s \'\n", rd_part);
        return -1;
	}
	void *addr = (void *)simple_strtoul(ram_addr_str, NULL, 16);
	u32 cnt = part.size;
	printf("Load from partition \' %s \' to address 0x%lx, count: %u block(s)\n", rd_part, (unsigned long)addr, cnt);
	if (blk_dread(dev_desc, part.start, cnt, addr) != cnt) {
		printf("%u blocks read from partition \' %s \' failed\n", cnt, rd_part);
		return -1;
	}
	printf("Read from partition \' %s \' done, total block(s) %u\n", rd_part, cnt);
	loff_t rd_size = cnt * part.blksz;
	env_set_hex("filesize", rd_size);
	return 0;
}
#endif
/*
...
static int label_boot(cmd_tbl_t *cmdtp, struct pxe_label *label)
{
...
#ifdef ENABLE_OPENHARMONY_RAMDISK_PART
	if (switch_ohos_boot_mode()) {
		if (load_openharmony_rd_to_ram(OHOS_PART_UPDATER) < 0) {
			printf("FATAL error: cannot load updater image to ram\n");
			return 1;
		}
 ...
 #endif // ENABLE_OPENHARMONY_RAMDISK_PART
	if (label->initrd) {
		...
		strcat(bootm_argv[2], ":");
		strncat(bootm_argv[2], env_get("filesize"), 9);
	} else if (ENABLE_OPENHARMONY_RAMDISK_PART) {
		if (load_openharmony_rd_to_ram(OHOS_PART_RAMDISK) < 0) {
			printf("FATAL error: cannot load ramdisk image to ram\n");
			return 1;
		}
		bootm_argv[2] = initrd_str;
		strncpy(bootm_argv[2], env_get("ramdisk_addr_r"), 18);
